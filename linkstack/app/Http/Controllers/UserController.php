...
class UserController extends Controller {
  ...
  public function littlelink(request $request) {
    ...
    $userinfo = User::select('id', 'name', 'littlelink_name', 'littlelink_description', 'theme', 'role', 'block', 'reward_business_id')->where('id', $id)->first();
  ...
  //Show add/update form
  public function AddRewardProgram($business_id = 0) {
    $data['businessId'] = '';
    return view('studio/add-reward', $data);
  }
  ...
  public function showPage(request $request) {
    ...
    $data['pages'] = User::where('id', $userId)->select('littlelink_name', 'littlelink_description', 'image', 'name', 'reward_business_id')->get();
  ...
  public function showProfile(request $request) {
    ...
    $data['profile'] = User::where('id', $userId)->select('name', 'email', 'role', 'reward_business_id')->get();
  ...
  public function addWallet(Request $request) {
    ...
    Log::info("Map connected Web3 Wallet for $userId: $walletAddress");

    if (!$walletAddress || strlen($walletAddress) == 0) {
      return response()->json([
        'status' => 'ERROR',
    ...
    UserWallet::upsert([
      ['user_id' => $userId, 'wallet_address' => $walletAddress, 'chain_id' => 1],
    ], ['chain_id']);
